...
```mermaid
flowchart TD
  A[Claude Code / Cursor calls MCP tool: apex.run] --> B[APEX Orchestrator]

  B --> C{mode}
  C -->|text| T1[Generate N text variants (ensemble)]
  T1 --> T2[Convergence scoring + select best]
  T2 --> T3[CoT audit (block on leakage)]
  T3 --> T4[Adversarial review (LLM)]
  T4 --> T5{known_good_baseline?}
  T5 -->|yes| T6[Compute baseline similarity; downgrade high_verified if low]
  T5 -->|no| T7[Skip]
  T6 --> T8[Policy verdict]
  T7 --> T8
  T8 --> OUT[Return verdict + output + metadata]

  C -->|code| C1[Generate N code variants (ensemble)]
  C1 --> C2[Convergence scoring + select best]
  ...
  C7 --> C10[Adversarial review + Doc-only inspection (parallel)]
  C9 --> C10
  ...
  C13 --> C14
  C14 -->|yes| C15[Compute baseline similarity; downgrade high_verified if low]
  C14 -->|no| C16[Skip]
